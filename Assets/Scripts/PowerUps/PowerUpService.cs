        ...
        private PowerUpController FetchPowerUp(PowerUpType typeToFetch)
        {
            ...
            switch (typeToFetch)
            {
                case PowerUpType.Shield:
                    return (PowerUpController)powerUpPool.GetPowerUpController<Shield>(fetchedData);
                case PowerUpType.DoubleTurret:
                    return (PowerUpController)powerUpPool.GetPowerUpController<DoubleTurret>(fetchedData);
                case PowerUpType.RapidFire:
                    return (PowerUpController)powerUpPool.GetPowerUpController<RapidFire>(fetchedData);
                default:
                    throw new Exception($"Failed to Create PowerUpController for: {typeToFetch}");
            }
        }

        public void ReturnPowerUpToPool(PowerUpController powerUpController) => powerUpPool.ReturnItem(powerUpController);
